    ...
    try:
        ...
        while True:
            try:
                ...
                # ✅ 使用全部已有数据（不扩充到全年）
                # 添加月份列（用于后续统计）
                trajectory_df['month'] = trajectory_df['datetime'].dt.month
                ...
                # 使用处理后的数据（已经是按时间排序的）
                processed_traj = trajectory_df.copy()

                processed_traj['vehicle_id'] = vehicle_id

                # 计算日期范围（用于获取气象数据）
                start_date = processed_traj['datetime'].min().strftime('%Y-%m-%d')
                end_date = processed_traj['datetime'].max().strftime('%Y-%m-%d')

                # 放入数据队列
                data_queue.put({
                    'vehicle_id': vehicle_id,
                    'trajectory': processed_traj,
                    'start_date': start_date,
                    'end_date': end_date,
                })

                print(f"[准备Worker-{worker_id}] ✅ {vehicle_id}: {len(processed_traj):,} records", flush=True)

                # 清理内存
                del trajectory_df, processed_traj
...
CONFIG = {
    ...
    'output': {
        ...
        'output_dir': '/data2/hcr/evipv/output_ev_charge',
    },
}
